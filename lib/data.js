...
// Reject anything that isn't http(s) or mailto so reference URLs from
// editor-supplied JSON can't smuggle in a javascript: scheme that would
// execute when a user clicks the link.
...
function normalizeSpecies(s) {
  ...
  return {
    ...
    references: refs.map((r) => ({ ...r, url: safeUrl(r.url) })),
  };
}
